...
Usage:
  npx tsx src/cli.ts <audio-file> [options]
...
Examples:
  ...
  npx tsx src/cli.ts song.mp3 --splitter dummy --stems vocals,drums --output ./out

  # Split with lalal splitter
  npx tsx src/cli.ts song.mp3 --splitter lalal --stems vocals,drum --api-key YOUR_KEY

  # Split and generate charts
  npx tsx src/cli.ts song.mp3 --splitter dummy --stems vocals --chart
`);
}
...
function parseArgs(args: string[]): CliOptions & { listStems?: boolean; help?: boolean } {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === "--help" || arg === "-h") {
            ...
            options.mergeFiles = [];
            // Collect all following non-flag arguments as chart file paths
            while (i + 1 < args.length && !args[i + 1]!.startsWith("-")) {
                options.mergeFiles.push(args[++i]!);
            }
            if (options.mergeFiles.length === 0) {
                console.error("Error: --merge requires at least one .chart file path");
                process.exit(1);
            }
        } else if (arg === "--merge-output") {
            const value = args[++i];
            if (!value) {
                console.error("Error: --merge-output requires a value");
                process.exit(1);
            }
...
function runMerge(chartPaths: string[], outputPath: string, options: CliOptions): void {
    console.log(`\nMerging ${chartPaths.length} chart files...`);
    const charts = chartPaths.map((p) => {
        const resolved = path.resolve(p);
        if (!fs.existsSync(resolved)) {
            console.error(`Error: Chart file not found: ${resolved}`);
            process.exit(1);
        }
        return parseChart(fs.readFileSync(resolved, "utf8"));
    });

    const songOverrides: Record<string, string> = {};
    if (options.songName) songOverrides.Name = options.songName;
    if (options.songArtist) songOverrides.Artist = options.songArtist;
    if (options.songGenre) songOverrides.Genre = options.songGenre;
    if (options.charterName) songOverrides.Charter = options.charterName;

    const merged = mergeCharts(charts, {
        songOverrides: Object.keys(songOverrides).length > 0 ? songOverrides : undefined,
    });

    const resolvedOutput = path.resolve(outputPath);
    fs.mkdirSync(path.dirname(resolvedOutput), { recursive: true });
    fs.writeFileSync(resolvedOutput, writeChart(merged), "utf8");
    console.log(`Merged chart written to: ${resolvedOutput}`);

    const trackNames = [...merged.tracks.keys()].sort();
...
async function main(): Promise<void> {
    ...
    // Standalone merge mode: merge existing .chart files and exit
    if (options.mergeFiles && !options.chart) {
        const mergeOutput = options.mergeOutput ?? path.join(options.outputDir, "merged.chart");
        runMerge(options.mergeFiles, mergeOutput, options);
        process.exit(0);
    }
        ...
        if (options.chart) {
            ...
            for (const stemResult of result.stems) {
                try {
                    ...
                    chartPaths.push(chartResult.chartPath);
                } catch (chartError) {
                    console.error(
